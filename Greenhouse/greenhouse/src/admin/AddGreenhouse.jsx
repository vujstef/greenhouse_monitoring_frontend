...
const AddGreenhouse = () => {
    ...
    const handleCreateGreenhouse = async (formData) => {
        try {
            if (checkRole(localStorage.getItem('roleId')) === 'admin') {
                const responseData = await axiosInstance.post(`Greenhouse/admin/createGreenhouse/${id}`, formData, {
                    headers: {
                        Accept: 'application/json',
                        Authorization: `Bearer ${localStorage.getItem('token')}`,
                    },
                });
                toast.success("Plastenik je uspješno kreiran.", { autoClose: 1500 })
                setTimeout(() => {
                    navigate('/home');
                }, 2000)
            } else {
            ...
            <MDBContainer fluid className='d-flex align-items-center justify-content-center bg-image' style={{ backgroundImage: `url(${image})` }}>
                ...
                <MDBCard className='m-5' style={{ maxWidth: '600px' }}>
                    <MDBCardBody className='px-5'>
                        <h2 className="text-uppercase text-center mb-5">Dodaj plastenik</h2>
                        ...
                        <MDBBtn className='mt-4 mb-4 w-100 gradient-custom-4' size='lg' onClick={handleSubmit(handleCreateGreenhouse)}>Dodaj</MDBBtn>
