    ...
    const handleUpdateUser = async (formData) => {
        try {
            if (checkRole(localStorage.getItem('roleId')) === 'admin') {
                const response = await axiosInstance.put(`Greenhouse/admin/updateUser/${id}`, formData, {
                    headers: {
                ...
                toast.success("Korisnik je uspješno ažuriran.", { autoClose: 1500 })
                setTimeout(() => {
            ...
            <MDBContainer fluid className='d-flex align-items-center justify-content-center bg-image' style={{ backgroundImage: `url(${image})` }}>
                ...
                <MDBCard className='m-5' style={{ maxWidth: '600px' }}>
                    <MDBCardBody className='px-5'>
                        <h2 className="text-uppercase text-center mb-5">Ažuriraj korisnika</h2>
                        ...
                        <Link to="/home/admin/getAllUser" className="d-block text-center mt-3">Vrati se nazad</Link>
                    </MDBCardBody>
                </MDBCard>
            </MDBContainer>
            <Footer></Footer>
